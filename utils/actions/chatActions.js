...
export const removeUserFromChat = async (
  ...
  const messageText =
    userLoggedInData.userId === userToRemoveData.userId
      ? `${userLoggedInData.firstName} ${userLoggedInData.lastName} left the group chat`
      : `${userLoggedInData.firstName} ${userLoggedInData.lastName} removed ${userToRemoveData.firstName} ${userToRemoveData.lastName} from the group chat`;
...
export const addUsersToChat = async (
  ...
  const messageText = `${userLoggedInData.firstName} ${userLoggedInData.lastName} invited ${userAddedName} ${moreUsersMessage}to the group chat`;
  await sendInfoMessage(chatData.key, userLoggedInData.userId, messageText);
};
